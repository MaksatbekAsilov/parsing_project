...
# @app.get("/prices/{source}", response_model=List[CryptoPrice])
# def get_prices_by_source(source: str, db: Session = Depends(get_db)):
#     # Проверка, какой источник был выбран и выполнение соответствующего запроса
#     if source == "vbr":
...
#     else:
#         raise HTTPException(status_code=400, detail="Invalid source, choose 'vbr', 'investing', or 'bitinfo'")

#     if not prices:
#         raise HTTPException(status_code=404, detail=f"No prices found for source {source}")
...
#     if source in (None, "VBR"):
#         queries.append(db.query(
#             VBRPrice.currency, 
#             VBRPrice.price, 
...
#     if not results:
#         raise HTTPException(status_code=404, detail="Нет валют в указанном диапазоне")
...
#     # Проверка на существование email (email должен быть уникальным)
